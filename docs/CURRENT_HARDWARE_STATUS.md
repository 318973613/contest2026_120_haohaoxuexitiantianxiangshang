...
| 唤醒词 | **云端监听运行，完整交互待验** | 本候选统一为“你好小米”，兼容旧“小维”及标点变体；自动开页和请求状态机已通过主机检查。仍是联网批量 ASR，不是离线唤醒。 |
| 对话速度 | **低延迟候选已打包，板端耗时未测** | `mimo-v2.5` 关闭思考、限制输出，去掉云端唤醒应答；等待慢回复期间不重新录音。 |
| 提醒与主页圆环 | **提醒本体用户确认可用，圆环联动待真机验收** | 新候选显示最近提醒的真实 deadline，支持异步取消和专注显示恢复；真实 LVGL/调度模块主机检查通过。 |
| NTP 时钟 | **最新日志已显示 2026，持续同步待验** | 9/6 日志 UNIX 时间与日期一致；不能再笼统写“板子仍是 1970”。旧伪造时钟已移除；没有新增 RTC/NTP 端到端验收。 |
...
2. 冷启动验证开机听感、串口和基本触摸；连续完成至少三轮语音对话并切换其他页面。
3. 在主页说“你好小米”，验证自动开页与短提示音；问“一条学习建议”，等待期间不得重复录音发问。
4. 设置 30 秒提醒后返回主页，验证圆环、到期提醒和专注恢复；再设置一分钟提醒验证取消。
5. 验证快速松手、识别时返回、失败后重试；若再失败，收集脱敏 FT5X06/TLS 诊断。天气和时钟另行验收，不用某一项成功代替整体通过。
